...
import { DEFAULT_LIVE_API_MODEL, AVAILABLE_VOICES, getVoicePersonaLabel } from '@/lib/constants';
    ...
    <aside className={c('sidebar', 'drawer', 'settings-drawer', { open: isSidebarOpen })}>
      ...
      <div className="p-6 flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Beatrice Persona</label>
          <textarea
            value={systemPrompt}
          ...
          >
            {AVAILABLE_VOICES.map((availableVoice) => (
              <option key={availableVoice} value={availableVoice}>
                {getVoicePersonaLabel(availableVoice)}
              </option>
            ))}
          </select>
        </div>
